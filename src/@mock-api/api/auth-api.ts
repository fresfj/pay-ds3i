...
let usersApi = mockApi.components.examples.auth_users
  .value as unknown as UserAuthType[]
...
export const authApiMocks = (mock: ExtendedMockAdapter) => {
  ...
  ): { access_token: string; user: User } | null {
    ...
    if (responseUrl.startsWith('/mock-api') && authorization) {
      ...
      if (newAccessToken) {
        response.headers['New-Access-Token'] =
          newAccessToken.access_token as string
      }
      return response
    }
    return response

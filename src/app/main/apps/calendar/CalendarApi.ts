  ...
  .injectEndpoints({
    endpoints: build => ({
      ...
      getCalendarLabels: build.query<
        GetCalendarLabelsApiResponse,
        GetCalendarLabelsApiArg
      >({
      ...
      createCalendarLabel: build.mutation<
        CreateCalendarLabelApiResponse,
        CreateCalendarLabelApiArg
      >({
        query: Label => {
          return {
      ...
      updateCalendarLabel: build.mutation<
        UpdateCalendarLabelApiResponse,
        UpdateCalendarLabelApiArg
      >({
        query: Label => ({
      ...
      deleteCalendarLabel: build.mutation<
        DeleteCalendarLabelApiResponse,
        DeleteCalendarLabelApiArg
      >({
        query: id => ({
...
export const selectFilteredEvents = (events: Event[]) =>
  createSelector([selectSelectedLabels], selectedLabels => {
    return events.filter(item =>
      selectedLabels.includes(item?.extendedProps?.label as string)
    )
  })

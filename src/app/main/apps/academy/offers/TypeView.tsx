...
import { useGetAcademyCashbackItemQuery } from '../AcademyApi';
...
function TypeView() {
	...
	const {
		...
		isLoading,
		isError
	} = useGetAcademyCashbackItemQuery(id, { skip: !id });
		...
		<>
			<Box
				...
				sx={{
					...
					backgroundImage: `url('assets/images/etc/cashback-1080.webp'})`,
				}}
			>
				<img
					className="absolute inset-0 object-cover object-top w-full h-full"
					src={'assets/images/etc/cashback-1080.webp'}
					alt="user background"
				/>
			...
			<div className="relative flex flex-col flex-auto items-center p-24 pt-0 sm:p-48 sm:pt-0">
				<div className="w-full max-w-3xl">
					...
					<div className="flex flex-col space-y-32">
						{offer.max &&
							<figure className="mt-10">
								<blockquote className="text-center text-xl font-semibold leading-8 text-gray-900 sm:text-2xl sm:leading-9">
									Até <span className="inline-flex items-center font-bold text-24 px-10 py-2 rounded-full tracking-wide uppercase bg-green-50 text-green-800 dark:bg-green-600 dark:text-green-50">{offer.max}%</span> de volta
								</blockquote>
							</figure>
						}
						...
						{offer.description && (
							<div className="flex items-center">
								...
								<div className="ml-24 leading-6">{offer.description}</div>
							</div>
						)}
						...
						{offer.rule && (
							<div className="flex">
								...
								<div
									...
									dangerouslySetInnerHTML={{ __html: offer.rule }}
								/>
							</div>
						)}
					...
					<Box className="flex justify-center items-center mt-18 py-24 pr-16 pl-4 sm:pr-48 sm:pl-36">
						<Button
							className='h-48 w-3/6	bg-green-600 font-semibold text-white hover:bg-green-500 rounded-lg shadow-md hover:scale-110 transition delay-150 duration-300 ease-in-out py-38'
							size="large"

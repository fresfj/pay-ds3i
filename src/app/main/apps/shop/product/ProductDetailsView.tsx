...
import { ProductDetailsSkeleton } from 'src/app/main/pages/product/components/ProductItemSkeleton';
...
export function ProductDetailsView({ product, error, loading, onOpen }: Props) {
  const { products } = useSelector(itemsCartSelector)
  ...
  return (
    <Container sx={{ mt: 5, mb: 10 }}>
        ...
        <Grid xs={12} md={6} lg={5}>
          {product && (
            <ProductDetailsSummary
              product={product}
              items={products}
              disableActions={!product?.quantity}
            />
          )}
        </Grid>
      </Grid>
      ...
      {totalItems > 0 &&
        <CartBottom cart={cart} total={total} totalItems={totalItems} onOpen={() => onOpen(true)} />
      }
    </Container>
  );
}

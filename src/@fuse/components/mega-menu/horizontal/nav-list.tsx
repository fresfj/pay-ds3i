...
import { usePathname } from '@fuse/hooks/use-pathname';
...
export function NavList({ data, render, slotProps, enabledRootRedirect, cssVars }: NavListProps) {
  ...
  const active: any = useActiveLink(data.path, !!data.children);
  ...
  const renderNavItem = (
    <NavItem
      ...
      externalLink={FuseUtils.isExternalLink(data.path)}
...
function useActiveLink(path: string, arg1: boolean) {
